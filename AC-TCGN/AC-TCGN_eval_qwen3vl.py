...
def tokenize_labels(processor, labels: List[str]) -> torch.Tensor:
    # labels are plain text answers; tokenize without adding special multimodal tokens
...
def patch_mlp_forward(model, activation_mask):
    """
    activation_mask: list[tensor] length = n_layers, each tensor holds neuron indices (to be zeroed)
    """
    ...
    for i, layer_mask in enumerate(activation_mask):
        ...
        if layer_mask is None:
            continue
        mlp.forward = MethodType(factory(layer_mask.to(next(model.parameters()).device)), mlp)
...
def main():
    ...
    ap.add_argument("--Evall_path", type=str, default=None)
    ...
    # Decoder-only models (e.g., Qwen) should use left padding for correct batched generation.
    ...
    if args.activation_mask:
        ...
        patch_mlp_forward(model, activation_masks)
    ...
    print(f"task={args.task}  metric={s:.6f}  masked={bool(args.activation_mask)}")
